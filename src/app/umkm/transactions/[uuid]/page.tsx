...
const ListItem = ({ name, value }: { name: string; value: any }) => {
  return (
    <div className="flex justify-between mb-3">
      <p>{name} :</p>
      <p className="font-medium text-black-2 dark:text-white">{value}</p>
    </div>
  );
};
...
export default function page({ params }: { params: { uuid: string } }) {
  ...
  return !isCheck ? (
    ...
    <>
      ...
      <div className="grid md:grid-cols-2 md:gap-9 p-5">
        <BasicCard title="Transaksi Produk">
          <div className="px-6.5 pt-5">
            <ListItem name="Tanggal Pembelian" value={values?.product?.name} />
            <ListItem name="Nama Produk" value={values?.product?.name} />
            <ListItem name="Nama Pembeli" value={values?.user?.name} />
            <ListItem name="Status Pembeli" value={values?.product?.name} />
          ...
          <div className="border-t border-stroke px-6.5 py-4 dark:border-strokedark">
            <ListItem name="Kontak Pembeli" value={values?.product?.name} />
          </div>
        ...
        <div>
          <div className="bg-white dark:bg-boxdark md:w-4/5 mx-auto mt-3 md:mt-0">
